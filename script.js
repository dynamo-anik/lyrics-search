...
//Show songs and artist details in DOM
...
function showData(data) {
  ...
  //   </ul>
  // `;
  //Easier Way unsing map() method and join() to turn it into sting
...
async function getLyrics(artist, songTitle) {
  ...
  result.innerHTML = `<h2><string>${artist}</strong> - ${songTitle}</h2>
  <span>${lyrics}</span`;
...
form.addEventListener('submit', (e) => {
  ...
  if (!searchTerm) {
    alert('Please enter somethng to search');
  } else {
